...
public class Destructor {
  ...
  public void mover() {
    ...
    if (movil) y += Math.sin(angulo) * velocidadY;
  }
  ...
  public void moverDerecha(Entorno entorno) {
    if (!movil) return;
    if (x + ancho / 2 < entorno.ancho()) x = x + velocidadX;
  }

  void moverIzquierda(Entorno entorno) {
    if (!movil) return;
    if (x - ancho / 2 > 0) x = x - velocidadX;
  }
